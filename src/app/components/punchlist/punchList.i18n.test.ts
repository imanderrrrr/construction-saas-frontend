...
import en from '../../../i18n/locales/en/punchList.json';
import es from '../../../i18n/locales/es/punchList.json';
...
describe('punchList locale coverage', () => {
  ...
  it('every workflow status and event type is labeled', () => {
    for (const status of ['OPEN', 'IN_PROGRESS', 'READY_FOR_REVIEW', 'REOPENED', 'CLOSED']) {
      expect(enMap[`status.${status}`], `status.${status}`).toBeTruthy();
    }
    for (const event of ['CREATED', 'ASSIGNED', 'READY', 'CONFIRMED', 'REJECTED', 'CLOSED', 'RETURNED_TO_PROGRESS']) {
